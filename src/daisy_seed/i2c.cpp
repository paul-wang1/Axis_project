...
// ESP32 I2C Address (AD0 pin = LOW)
const uint8_t ESP32_ADDRESS = 0x28;
...
/* 
 * Helper function to recieve raw data from the ESP32
 * @effects: Will block processes until sensor 14 bytes are read brom the sensor
 */
void ReadESP32Register(uint8_t reg, uint8_t arr[], uint8_t size) {
    i2c.ReceiveBlocking(ESP32_ADDRESS, arr, size, 100);
}
