...
ble_spp_server_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        MODLOG_DFLT(INFO, "\n");
         ...
         conn_handle_subscriber[event->disconnect.conn.conn_handle] = false;
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        MODLOG_DFLT(INFO, "subscribe event; conn_handle=%d attr_handle=%d "
         ...
         conn_handle_subscriber[event->subscribe.conn_handle] = true;
        return 0;
  ...
  int  ble_svc_gatt_handler(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR:
        ...
        MODLOG_DFLT(INFO, "data is %c", OS_MBUF_DATA(ctxt->om, int)[0]);
        break;
